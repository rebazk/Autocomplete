...
public class BinarySearchDeluxe {
    ...
    public static <Key> int firstIndexOf(Key[] a, Key key, Comparator<Key> c) {
        // If any of the arguments are null, throw exception (Corner Case).
        if (a == null || key == null || c == null) {
            throw new NullPointerException("a, key, or c is null");
        }
        // Same binary search expect when a[mid] matches key, instead of returning mid,
        // set index to mid, index (initialized to -1), and adjust hi to mid - 1.
        int lo = 0;
        int hi = a.length - 1;
        int index = -1;
    ...
    public static <Key> int lastIndexOf(Key[] a, Key key, Comparator<Key> c) {
        ...
        // Same binary search expect when a[mid] matches key, instead of returning mid,
        // set index to mid, index (initialized to -1), and adjust lo to mid + 1.
        int lo = 0;
        int hi = a.length - 1;
        int index = -1;
        // Handling empty arrays.
